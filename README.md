...
Another good way to do it is with any suitable WatchdogTimer such as the one referenced in comment.
...
The flexibility of this approach can be demoed by waiting for a restartable 1-second interval to expire before showing an overlay with a mock generated sql command. Then (for this demo) the overlay itself is timed with another instance (not shown in the snippet above). Finally a third WDT instance is attached to the click button and likewise displays a message when the user stops clicking for a certain amount of time. Fun with timers, anyway, and lots of ways to achieve the objective.
...
```
  ...
  [1]: https://i.stack.imgur.com/EPtTp.png